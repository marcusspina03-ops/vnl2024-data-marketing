# =============================================================================
# VNL 2024 - DASHBOARD PLOTLY DASH
# Volleyball Nations League | MSc2 Manager Data Marketing
# =============================================================================
#
# Ce script crée un dashboard interactif avec Dash et Plotly.
#
# Prérequis :
#   - Avoir exécuté pipeline.py au moins une fois (pour créer player_scores)
#   - Fichier .env avec DB_HOST, DB_USER, DB_PASSWORD, DB_NAME
#   - pip install dash plotly mysql-connector-python pandas python-dotenv
#
# Lancement :
#   python app.py
...
# =============================================================================
#
# On charge les données une seule fois au démarrage de l'application.
# Recharger à chaque interaction serait trop lent.
#
# pd.read_sql() exécute une requête SQL et retourne un DataFrame pandas
# directement, sans passer par fetchall() + pd.DataFrame().
#
# =============================================================================
...
def get_connection():
    """Crée et retourne une connexion MySQL depuis les variables .env"""
...
def load_dashboard_data():
    """
    Charge les données player_scores enrichies depuis MySQL.
    Jointure avec teams pour récupérer pays, continent, population et drapeau.
    """
    conn = get_connection()
    df = pd.read_sql("""
        SELECT
            ...
            t.population,
            t.flag_url,
            (2024 - ps.birth_year) AS age
        FROM player_scores ps
        JOIN teams t ON ps.team_code = t.code
    """, conn)
    conn.close()
    return df

def load_teams_data():
    """Charge les données équipes avec drapeaux depuis MySQL"""
    conn = get_connection()
    teams = pd.read_sql("""
        SELECT code, country, continent, population, flag_url
        FROM teams
        ORDER BY code
    """, conn)
    conn.close()
...
if __name__ == '__main__':
    print("\n=== VNL 2024 - Dashboard Player Performance ===")
    print(f"  {len(df)} joueurs chargés depuis MySQL")
